...
BUILDCOMMAND := go build -trimpath -ldflags "-s -w -X github.com/clarkezone/${BIN_NAME}/pkg/config.VersionHash=${VERSION_HASH} -X github.com/clarkezone/${BIN_NAME}/pkg/config.VersionString=${VERSION_STRING}" -tags "osusergo netgo static_build"
temp = $(subst /, ,$@)
os = $(word 1, $(temp))
arch = $(word 2, $(temp))
label = $(word 3, $(temp))
